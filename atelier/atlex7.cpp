...
public:
    ...
    vecteur3d somme(vecteur3d v) {
        return vecteur3d(x + v.x, y + v.y, z + v.z); // retourn la somme des vecteurs 
    }

    //Le produit scalaire de deux vecteurs
    float produit(vecteur3d v) {
        return x * v.x + y * v.y + z * v.z; // retourn le produit des vecteurs 
    }

    //tester si deux vecteurs ont les memes composantes
    bool coincide(vecteur3d v) {
        //v1.coincide(v2)
        return (x == v.x && y == v.y && z == v.z); // retourn les  vecteurs  coincide 
    }

    //Retourner la norme du vecteur
    float norme() {
        return sqrt(x * x + y * y + z * z); // retourn la sqrt des vecteurs 
    }
